...
import ImgProgressWheel from 'assets/img/progress-wheel.svg';
...
function Loading(props: ILoadingProps) {
  ...
  const iconLength =
    typeof iconSize === 'number' ? iconSize : iconLengthDef[iconSize];
  ...
  return (
    <StyledLoading
      ...
      iconColor={iconColor}
    >
      {!noIcon && <ReactSVG src={ImgProgressWheel} />}
      {message && (
        <StyledMessage messageStyle={messageStyle}>{message} </StyledMessage>
      )}
    </StyledLoading>
  );
}
